...
class online_svd_calculator(object):
    ...
    def initialize(self,A):
        ...
        # Compute SVD of r with TF - v is not transposed like numpy  - step 2 section II
        self.di, ui, _ = tf.linalg.svd(r) 

        self.di = self.di.numpy()
        ui = ui.numpy()
    ...
    def incorporate_data(self,A):
        ...
        udashi, ddashi = tf.linalg.qr(m_ap)

        udashi = udashi.numpy()

        # Section III B 3(b):
        dtildei, utildei, _ = tf.linalg.svd(ddashi)

        dtildei = dtildei.numpy()
        utildei = utildei.numpy()
